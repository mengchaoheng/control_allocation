...
for i = 1:m
    for j = 1:k
        ...
        plot(t, sign(K_u)*max(abs(squeeze(Deriv_u_v(i,j,:)))), 'm-', 'LineWidth', 0.5);hold on;
        plot(t, K_status*max(abs(squeeze(Deriv_u_v(i,j,:)))), 'c-', 'LineWidth', 0.5);hold on;
        xlabel('Time (s)');
        ylabel(sprintf('du/dv(%d,%d)', i, j));
        legend('Deriv_{LP}', 'Deriv_{inv}', 'Deriv_{rest}','k_u*10', 'k_status','Location','northwest','NumColumns', 5);
        axis([1 3 -inf inf]);
        grid on;
...
plot(t, K_status, '-','Color', d_color, 'LineWidth', 1.8);hold on;
axis([1.8 3 -inf inf]);