...
%%
% % 时间向量
% t = linspace(0, 10, 1000);  % 从0到10秒，1000个点
% N=size(t,2);
% % 构造单位球上的向量 v(t)
% theta = 2 * pi * 1 * t;       % 方位角（变化速度0.2Hz）
% phi = pi/4 * sin(2 * pi * 1 * t);  % 仰角也随时间变化（0.1Hz正弦变化）
% 
% % 球坐标转直角坐标（单位球面）
% v_x = 40*cos(phi) .* cos(theta);
% v_y = 40*cos(phi) .* sin(theta);
% v_z = 60*sin(phi);
% 
% % 拼成矩阵，每列是 v(t)
% v = [v_x; v_y; v_z];
...
for i = 1:m
    for j = 1:k
        ...
        % plot(t, K_u, 'm-', 'LineWidth', 0.5);hold on;
        % plot(t, K_status, 'c-', 'LineWidth', 0.5);hold on;
        xlabel('Time (s)');
        ylabel(sprintf('du/dv(%d,%d)', i, j));
        legend('Deriv_{LP}', 'Deriv_{inv}', 'Deriv_{rest}');%,'k_u', 'k_status'
        axis([1 3 -inf inf]);
        grid on;
...
K=diag([10,10,12]);
u1=2*(1/2)*norm((K)\B, 2);
u2=2*(1/2)*norm(pinv(B)*K*(-K), 2);
u3=2*(1/2)*norm(pinv(B)*K*B, 2);
1/(u3+u1*u2)