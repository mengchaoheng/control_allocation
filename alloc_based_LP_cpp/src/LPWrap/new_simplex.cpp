...
void BoundedRevisedSimplex( Eigen::MatrixXf& A,
    ...
    while ((!done || !unbounded) && (itlim > 0)) {
        ...
        if (minr >= 0) {
            ...
            // std::cout << "BoundedRevisedSimplex done"<< std::endl; //Check this condition
            break; // 退出循环
        }
        ...
        {
            unbounded = true;
            // std::cout << "BoundedRevisedSimplex Solution is unbounded"<< std::endl; // Check this condition
            break;
        }
...
void DPscaled_LPCA(const Eigen::VectorXf& yd, const Eigen::MatrixXf& B, const Eigen::VectorXf& uMin, const Eigen::VectorXf& uMax, int itlim, Eigen::VectorXf& u, int& errout)
{
    ...
    int iy;
    float my = yd.array().abs().maxCoeff(&iy);
    if(my<tol)
    {
        errout = -1; 
        return;
