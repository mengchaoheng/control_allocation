...
static real_T (*l_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u, const
  emlrtMsgIdentifier *parentId))[9];
static real_T (*m_emlrt_marshallIn(const emlrtStack *sp, const mxArray *Wu,
  ...
  const char_T *identifier))[81]
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = n_emlrt_marshallIn(sp, emlrtAlias(Wu), &thisId);
 ...
 * Return Type  : void
 */
void wls_alloc_m_api(const mxArray * const prhs[12], int32_T nlhs, const mxArray
                     *plhs[1])
{
  ...
  Wu = m_emlrt_marshallIn(&st, emlrtAlias(prhs[7]), "Wu");
  ud = e_emlrt_marshallIn(&st, emlrtAlias(prhs[8]), "ud");
  imax = i_emlrt_marshallIn(&st, emlrtAliasP(prhs[9]), "imax");
  gam = i_emlrt_marshallIn(&st, emlrtAliasP(prhs[10]), "gam");
  only_plim = o_emlrt_marshallIn(&st, emlrtAliasP(prhs[11]), "only_plim");

  /* Invoke the target function */
  wls_alloc_m(*B, *v, *u, *p_limits, *v_limits, T, *Wv, *Wu, *ud, imax, gam,
              only_plim);
...
/*
 * File trailer for _coder_wls_alloc_m_api.c
 *
 * [EOF]
 */
