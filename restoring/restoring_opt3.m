...
function [u_rest,K_u,K_status,Deriv_ku_u]  = restoring_opt3(B,u,uMin,uMax)
...
% if (rank(B_aug)==k)
    ...
    % if isOuted  
        ...
        for i=1:m
            ...
            if(u_r(i)>0)
                tmp=uMax_new(i)/u_r(i);
            else
                tmp=uMin_new(i)/u_r(i);
            end
            if(tmp<K_max_ur) % find smaller
                K_max_ur=tmp;
                max_index=i;
                if(u_r(i)>0)
    ...
    K_max=min([K_max_ur;1]);
    if K_max < 1 % or using the min one, but not rubostly
        K_u=K_max*K_opt;
        K_status=2;
