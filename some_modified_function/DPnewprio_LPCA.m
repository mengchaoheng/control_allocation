function [u,errout,error,errore,errord] = DPnewprio_LPCA(yd,ye,B,uMin,uMax)
...
% [u_all,errout_all,rho] = DP3_LPCA(yd+ye,B,uMin,uMax);
errore=[0;0;0];
errord=[0;0;0];
...
%     [ue,erroute,rhoe] = DP3_LPCA(ye,B,uMin,uMax);
    ...
    if rhoe>1 %ye严格在AMS内，注意分配yd时标称位置已经变为ue
        uMin=uMin-ue;
        uMax=uMax-ue;
%         [ud,erroutd,rhod] = DP3_LPCA(yd,B,uMin,uMax); % rhod应大于1
