...
static void wls_alloc_m(const double B[27], const double v[3], double u[9],
 ...
 * Return Type  : void
 */
void dyn_alloc_m(const double B[27], const double v[3], double u[9], const
                 double p_limits[2], const double v_limits[2], double T, const
                 double Wv[9], const double W1[81], const double W2[81], const
                 double S[27], double imax, double gam, boolean_T only_plim)
{
  int k;
  int i1;
  static double W1sq[81];
  static double W2sq[81];
  int i2;
  static double Wu[81];
  static double b_S[9];
  static double b_W1sq[9];
  static double b_W2sq[9];
  static double c_W1sq[81];
  static double dv6[9];
  for (k = 0; k < 9; k++) {
    for (i1 = 0; i1 < 9; i1++) {
      W1sq[k + 9 * i1] = 0.0;
      W2sq[k + 9 * i1] = 0.0;
      for (i2 = 0; i2 < 9; i2++) {
        W1sq[k + 9 * i1] += W1[k + 9 * i2] * W1[i2 + 9 * i1];
        W2sq[k + 9 * i1] += W2[k + 9 * i2] * W2[i2 + 9 * i1];
      }
    }
  }
  ...
  for (k = 0; k < 9; k++) {
    ...
    b_W2sq[k] = 0.0;
    for (i1 = 0; i1 < 9; i1++) {
      b_W1sq[k] += W1sq[k + 9 * i1] * b_S[i1];
      b_W2sq[k] += W2sq[k + 9 * i1] * u[i1];
    }

    dv6[k] = b_W1sq[k] + b_W2sq[k];
  }

  for (k = 0; k < 81; k++) {
    c_W1sq[k] = W1sq[k] + W2sq[k];
  }

  mldivide(c_W1sq, dv6);
  wls_alloc_m(B, v, u, p_limits, v_limits, T, Wv, Wu, dv6, imax, gam, only_plim);
}
...
/*
 * File trailer for dyn_alloc_m.c
 *
 * [EOF]
 */
